...
class TestLoginUser(APITestCase):
    ...
    def test_user(self):
        ...
        response = self.client.post(
            url, {"email": "testemail@gmail.com", "password": "123"}
        )
...
class TestRegisterUser(APITestCase):
    ...
    def test_missing(self):
        response = self.client.post(
            self.url,
            {"email": "email@gmail.com", "first_name": "first", "family_name": "last"},
        )
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
...
class PasswordResetTest(APITestCase):
    ...
    def test_reset_password_success(self, mock_redis_client):
        ...
        reset_url = reverse(
        "users:reset-password", kwargs={"uidb64": uidb64, "token": token}
        )
        response = self.client.post(

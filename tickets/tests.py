...
class TicketTest(APITestCase):
    def setUp(self):
        self.user = User.objects.create_user(email='testemail@gmail.com', first_name= 'test_name', family_name= 'family_test', password = '123',is_active = True)
        self.client.force_authenticate(user=self.user)
        self.event = Event.objects.create(vendor=self.user, title = 'test', description = 'test_description', location='anywhere', event_date = timezone.now()+timedelta(days=20))
        self.ticket_type = TicketType.objects.create(ticket_tier = 'dummy', ticket_to_event = self.event, available_quantity= 100, total_quantity = 190, price = 10.90, sales_start_at = timezone.now(), sales_ended_at = timezone.now()+timedelta(days=10) )
        self.ticket = Ticket.objects.create(ticket_type = self.ticket_type, owner = self.user)
    ...
    def test_detail_other_user_ticket(self):
        user = User.objects.create_user(email='test_email@gmail.com', first_name= 'testname', family_name= 'familytest', password = '123!',is_active = True)
    ...
    def test_create_checkout_session_success(self, mock_stripe_create):
        mock_stripe_create.return_value = type('obj', (object,), {'url': 'https://checkout.stripe.com/test_url'})
        url = reverse('tickets:create-checkout-session', kwargs={'pk': self.ticket.uuid})
        response = self.client.post(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['checkout_url'], 'https://checkout.stripe.com/test_url',)
    ...
    def test_stripe_webhook_updates_ticket_status(self, mock_construct_event):
        ...
        response = self.client.post(url, data = {'dummy': 'data'}, format='json', HTTP_STRIPE_SIGNATURE='fake_signature')
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.ticket.refresh_from_db()

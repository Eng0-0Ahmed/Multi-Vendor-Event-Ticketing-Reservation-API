...
class TicketQuerySet(models.QuerySet):
    def expired_reservations(self, hold_minutes=10):
        ...
        return self.filter(status=Ticket.Status.RESERVED, reserved_at__lt=expiration_cutoff)

class TicketType(models.Model):
    uuid = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    ticket_tier = models.CharField(max_length=200)
    ticket_to_event = models.ForeignKey(Event, on_delete= models.CASCADE, related_name='ticket_types')
    ...
    class Meta:
        constraints = [
            models.CheckConstraint(condition = models.Q(available_quantity__lte=models.F('total_quantity')), name = 'avalible_quantity_to_total_quantity'),
            models.CheckConstraint(condition= models.Q(price__gte=0), name='price_gte_zero'),
        ]
    ...
    def reserve_ticket(self, user, hold_minutes=10):
        with transaction.atomic():
            ...
            now = timezone.now()
            if (now < locked_ticket_type.sales_start_at or now > locked_ticket_type.sales_ended_at):
            ...
            ticket = Ticket.objects.create(ticket_type=locked_ticket_type, owner=user, status = Ticket.Status.RESERVED, reserved_at=now,)
            return ticket
    def __str__(self):
...
class Ticket(models.Model):
    ...
    def release_expired_hold(self):
        with transaction.atomic():
            updated_count = Ticket.objects.filter(pk=self.pk, status=Ticket.Status.RESERVED).update(status=self.Status.CANCELLED)
            if updated_count > 0:
                TicketType.objects.filter(pk=self.ticket_type_id).update(available_quantity=models.F('available_quantity') + 1)

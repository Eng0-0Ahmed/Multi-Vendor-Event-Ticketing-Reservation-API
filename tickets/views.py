...
class MyTicketsListView(generics.ListAPIView):
  ...
  serializer_class = TicketSerializer
  def get_queryset(self):
    return Ticket.objects.filter(owner=self.request.user).select_related('ticket_type')
...
class CancelReservationView(APIView):
    ...
    def post (self, request, pk):
        ...
        if ticket.status != Ticket.Status.RESERVED:
            raise DRFValidationError('Cannot cancel a ticket that has already been purchased or cancelled')
        try:
            ticket_unreserve = ticket.release_expired_hold()
        except DjangoValidationError as e:
            raise DRFValidationError(detail=e.messages)
        return Response(TicketSerializer(ticket_unreserve).data, status=status.HTTP_200_OK)
...
class CreateCheckoutSessionView(APIView):
    ...
    def post(self, request, pk):
        try:
          ticket = Ticket.objects.get(uuid=pk, owner=request.user, status=Ticket.Status.RESERVED)
        except Ticket.DoesNotExist:
          return Response({'error': 'Reserved ticket not found or expired.'}, status=status.HTTP_404_NOT_FOUND)
        ...
        session = stripe.checkout.Session.create(
            ...
            line_items=[{
                'price_data': {
                    'currency': 'usd',
                    'product_data': {'name': f"{ticket.ticket_type.ticket_to_event.title} - {ticket.ticket_type.ticket_tier}",},
        ...
        if event['type'] == 'checkout.session.completed':
            ...
            if ticket_id:
                with transaction.atomic():
                    ticket = (Ticket.objects.select_for_update().filter(uuid=ticket_id).first())
...
class VerifyTicketView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response({'detail': 'Ticket not found or invalid UUID.'}, status=status.HTTP_404_NOT_FOUND,)
        if ticket.status == Ticket.Status.USED:
            return Response({'detail': 'Ticket has already been used! Entry denied.'}, status=status.HTTP_400_BAD_REQUEST,)
        if ticket.status != Ticket.Status.PURCHASED:
            return Response({'detail': f'Ticket cannot be verified (Status: {ticket.status}).'}, status=status.HTTP_400_BAD_REQUEST,)
